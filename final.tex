...
By linking our trainer with reinforcement learning code using \texttt{popen()}, \texttt{write()}, and \texttt{read()} in Python, we apply real-time reinforcement learning to our game playing. Every time a game state is generated, we feed it into our Q-Learning \texttt{Brain} object, and the \texttt{Brain} then outputs the optimal move for the game. Then a reward is calculated based on the move, and provided back to the \textt{Brain} as feedback.
...
\bibitem{2} \texttt{http://flask.pocoo.org/}

\bibitem{3} Hadi Pouransari & Saman Ghili. "AI algorithms for the game 2048".

\bibitem{4} \texttt{http://torch.ch/}

\bibitem{5} \texttt{https://en.wikipedia.org/wiki/Softmax\_function}

\bibitem{6} \texttt{https://github.com/blakeMilner/DeepQLearning}

\bibitem{7} \texttt{https://github.com/ov3y/2048-AI}
